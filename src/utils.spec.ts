...
describe('Testing getUrlFromSearchString utility', () => {
  ...
  test('for search query input it should return apropriate link', () => {
    expect(getUrlFromQuery('/url?q=https://www.youtube.com/Google&sa=U&ved=0ahUKEwjvz7ySg9XfAhVTBWMBHZxaCVUQFghEMA0&usg=AOvVaw3hj_MvlPj_z331AUw0E8P8')).toEqual('https://www.youtube.com/Google');
    expect(getUrlFromQuery('/search?q=google&safe=off&gl=US&pws=0&nfpr=1')).toEqual('https://google.com/search?q=google&safe=off&gl=US&pws=0&nfpr=1');
  });
});
